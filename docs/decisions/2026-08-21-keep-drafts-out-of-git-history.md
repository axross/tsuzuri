...
GitHub's recommended push rate is the binding constraint on this product's
write path — it bites well before any API rate limit — and an editor that
autosaves every few seconds would exceed it while a single person types.
Committing drafts would also fill the repository's history with hundreds of
commits per article, which degrades every clone and every tree operation for
as long as the repository exists, and there is no way to un-write them
afterwards.
...
be debounced to the interval that rate implies. Cross-device draft sync would need
