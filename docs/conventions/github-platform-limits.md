...
commits well clear of that ceiling.

The refusal is not graceful: GitHub answers an oversized create with HTTP 422
and the message that the input was too large to process, suggesting the blob
be created in a local clone and pushed instead — a remedy no serverless
function can take.
